if ( ! -e ${HOME}/.noEuclidLoginScript ) then
  ...
  if ( -r ${my_own_prefix}/bin/Euclid_group_setup.csh ) then
    set shellscr=${my_own_prefix}/bin/Euclid_group_setup.csh
  else
  ...
  # .login
  if ($?loginsh) then
    if ( -r ${my_own_prefix}/bin/Euclid_group_login.csh ) then
      set loginscr=${my_own_prefix}/bin/Euclid_group_login.csh
    else
