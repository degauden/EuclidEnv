    ...
    def run(self, args=sys.argv[1:]):
        ...
        if profiler:
            ...
            if sys.version_info < (2, 5):
                ...
                # This should be ok, as at this point the profiled script has
                # finished running
            ...
            if self.options.profdump != None:
                ...
                stats.strip_dirs().sort_stats(
                    self.options.profsortby).print_stats(self.options.profmaxlines)
                # Log the actual statistics
                self.log.warning(output.getvalue())

            # Now resetting stdout if it was swapped (on older version of
            # python)
            if stdoutSwapped:
...
class ConfigScript(PlainScript):
    ...
    def __init__(self, usage=None, version=None, parser=Parser,
        ...
        """ constructor of the Script. provides default options parser
        and logger """
        PlainScript.__init__(
            self, usage, version, parser, help_output, description)
    ...
    def setConfigFile(self, config_file=None, config_dir=None,
        ...
        if config_file:
            ...
            if not config_ext:
                config_ext = "conf"
        addConfigFileOptions(
            self.parser, config_file, config_dir, config_name, config_ext)
