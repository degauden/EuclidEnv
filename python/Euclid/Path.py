...
def envPathPrepend(pathname, dirname, env_dict=None, exist_check=False, unique=False):
    if not env_dict:
        env_dict = os.environ
    env_dict[pathname] = pathPrepend(
        env_dict[pathname], dirname, exist_check, unique)


def envPathAppend(pathname, dirname, env_dict=None, exist_check=False, unique=False):
    if not env_dict:
        env_dict = os.environ
    env_dict[pathname] = pathAppend(
        env_dict[pathname], dirname, exist_check, unique)
...
def multiPathUpdate(path, dirlist, interleaved=False, exist_check=False):
    ...
    if not interleaved:
        for d in dirlist:
            pathlist.extend(
                multiPathJoin(path, d, exist_check).split(os.pathsep))
    else:
        for p in path.split(os.pathsep):
            for d in dirlist:
                jd = os.path.join(p, d)
